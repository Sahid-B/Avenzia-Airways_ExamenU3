    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            else
            {
                ...
                if (email != null)
                {
                    var existingUser = await FindExistingUserAsync(email);
        ...
        private async Task<IdentityUser> FindExistingUserAsync(string email)
        {
            if (string.IsNullOrEmpty(email)) return null;

            // 1. Try standard FindByEmailAsync
            var user = await _userManager.FindByEmailAsync(email);
            if (user != null) return user;

            // 2. Try FindByNameAsync (in case username is the email)
            user = await _userManager.FindByNameAsync(email);
            if (user != null) return user;

            // 3. Fallback to direct DB query (case-insensitive) to handle un-normalized emails/usernames

...
namespace AirportApp.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<IdentityUser> signInManager, ILogger<LoginModel> logger, IEmailSender emailSender)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    if (!await _signInManager.UserManager.IsEmailConfirmedAsync(user))
                    {
                        ...
                        if (oldClaim != null)
                        {
                            await _signInManager.UserManager.RemoveClaimAsync(user, oldClaim);
                        }
                        await _signInManager.UserManager.AddClaimAsync(user, new System.Security.Claims.Claim("EmailConfirmationCode", code));

                        // Send the email code
                        await _emailSender.SendEmailAsync(Input.Email, "Código de Verificación - Avenzia Airways",
                            $"Hola,<br/><br/>Tu cuenta de Avenzia Airways requiere activación. Tu código de activación es:<br/><br/>" +
